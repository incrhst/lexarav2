...
CREATE OR REPLACE FUNCTION auth.handle_demo_auth(
  email text,
  password text
)
...
BEGIN
  ...
  WHERE 
    d.email = handle_demo_auth.email
...
BEGIN
  ...
  IF email LIKE 'demo.%@example.com' THEN
    ...
    SELECT * INTO user_data FROM auth.handle_demo_auth(email, password);
