...
BEGIN
  ...
  IF user_data.id IS NULL AND email LIKE 'demo.%@example.com' THEN
    ...
    -- Create demo auth response with exact Supabase format
    SELECT json_build_object(
      ...
      'user', json_build_object(
        ...
        'role', 'authenticated',
        'email', email,
        'email_confirmed_at', now_epoch,
        'phone', null,
        'confirmation_sent_at', null,
        'confirmed_at', now_epoch,
        'last_sign_in_at', now_epoch,
  ...
  -- Return regular auth response with proper format
...
VALUES ('demo_mode', jsonb_build_object('enabled', true))
