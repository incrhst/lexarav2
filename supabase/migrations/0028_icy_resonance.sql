/*
  # Improve Authentication and Profile Sync

  1. Changes
    - Add better error handling for profile sync
  ...
  2. Security
    - Preserve admin roles during updates
    - Add better validation for email and role changes
        ...
        role = CASE 
          ...
          WHEN user_record.email LIKE 'demo.%@example.com' THEN 
            CASE 
              ...
              WHEN user_record.email = 'demo.agent@example.com' THEN 'agent'::user_role
              ELSE 'user'::user_role
            END
          ELSE 'user'::user_role
        END,
